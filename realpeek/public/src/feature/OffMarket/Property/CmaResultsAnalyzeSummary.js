...
import CmaResultsAnalyzeSummaryMult from "./CmaResultsAnalyzeSummaryMult";
...
class CmaResultsAnalyzeSummary extends Component {
    ...
    renderBarGraph = (values, labels, title, type) => {
        ...
        return <div className="chart-wrapper" style={{
                    'height': 350,
                    'margin': '0 auto',
                    'width': '100%'
                }}>
  ...
  renderPieGraph = (values, labels, title, type) => {
        const data = {
            labels: labels,
            datasets: [
            {
    ...
    render() {
        ...
        if (cma.subject_property.mp_style == 7) {
            return <CmaResultsAnalyzeSummaryMult {...this.props} />
        }
...
export default CmaResultsAnalyzeSummary;