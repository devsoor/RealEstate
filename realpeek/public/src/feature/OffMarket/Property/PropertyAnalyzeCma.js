...
import { Row, Col, Card, CardTitle, CardSubtitle, CardBody, Form, FormGroup, Input, InputGroup, InputGroupAddon, Button, Alert, UncontrolledTooltip, UncontrolledCollapse } from 'reactstrap';
...
class PropertyAnalyzeCma extends Component {
    constructor(props) {
        super(props);
        this.state = this.props.assumptions;
    }

    handleChange = (e) => {
        var newState = {};
        newState[e.id] = e.value;
        if (e.id === "property_tax_rate") {
            if (e.value) {
                newState["property_tax_rate_auto"] = false;
            }
            else {
                newState["property_tax_rate_auto"] = true;
            }
        }
    ...
    renderMetric(id, name, subtitle, type, value, tip) {
        ...
        return <Card body id={id} className="bg-info">
    ...
    renderRent(cma_results) {
        if (cma_results.Result_EstMonthlyRent_Units) {
            ...
            return <CmaFormControl name='Est. Rent' id='Result_EstMonthlyRent' value={cma_results.Result_EstMonthlyRent} readOnly={true} type="currency" period="monthly" />
        }
    }

    renderRentInput(cma_results) {
        if (this.props.cma) {
            if (this.state['monthly_rent'] == 0) {
                this.state['monthly_rent'] = this.props.cma.params.monthly_rent
            }
        }
        if (cma_results.Result_EstMonthlyRent_Units) {
            return <div>
                ...
                </FormGroup>
                {
                    cma_results.Result_EstMonthlyRent_Units.map((unit) => {
                        ...
                        return <CmaFormControl name={rowName} id={id} key={id} value={this.state[id]} current={currentRent} type="currency" decimals={0} period="monthly" onChange={this.handleChange} />
                    })
                }
            </div>
        }
        else {
            return <CmaFormControl name='Est. Monthly Rent' id='monthly_rent' value={this.state.monthly_rent} type="currency" decimals={0} period="monthly" onChange={this.handleChange} />
        }
    }
    render() {
        ...
        var cma = property.cma;
        var cma_results = cma.cma.cma_results;
            ...
            <CardTitle className="bg-info border-bottom p-3 mb-0 text-white">Performance Metrics</CardTitle>
                ...
                <Card className="">
                  <CardTitle className="bg-success border-bottom p-3 mb-0 text-white">Edit Scenarios</CardTitle>
                    ...
                    <CardBody>
                    <Form>
                        <CmaFormControl name='Closing Costs' id='closing_costs_percent' value={this.state.closing_costs_percent} type="percent" decimals={1} onChange={this.handleChange} />
                        <CmaFormControl name='Down Payment' id='downpayment_percent' value={this.state.downpayment_percent} type="percent" decimals={1} onChange={this.handleChange} />
                        <CmaFormControl name='Mortgage Interest Rate' id='mortgage_interest_rate' value={this.state.mortgage_interest_rate} type="percent" decimals={3} onChange={this.handleChange} />
                        <CmaFormControl name='Maintenance' id='maintenance_percent' value={this.state.maintenance_percent} type="percent" decimals={0} onChange={this.handleChange} />
                        <CmaFormControl name='Improvements' id='improvements' value={this.state.improvements} type="currency" decimals={0} onChange={this.handleChange} />
                        <CmaFormControl name='Property Tax Rate' id='property_tax_rate' value={this.state.property_tax_rate} type="percent" decimals={3} onChange={this.handleChange} />
                        <CmaFormControl name='Vacancy Rate' id='vacancy_rate' value={this.state.vacancy_rate} type="percent" decimals={1} onChange={this.handleChange} />
                        <CmaFormControl name='Prop. Management' id='property_mgmt_percent' value={this.state.property_mgmt_percent} type="percent" decimals={1} onChange={this.handleChange} />
                        <CmaFormControl name='Insurance Rate' id='insurance_rate' value={this.state.insurance_rate} type="percent" decimals={1} onChange={this.handleChange} />
                        <CmaFormControl name='Purchase Price' id='purchase_price' value={this.state.purchase_price} type="currency" onChange={this.handleChange} />
  ...
  export default PropertyAnalyzeCma;