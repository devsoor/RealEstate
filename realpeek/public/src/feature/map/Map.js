...
class Map extends Component {
    ...
    zoomMapToFit = async () => {
        if (this.props.results && this.props.results.length > 0) {
            ...
            if (this.props.searchType === "poi" && this.props.poi) {
                result = {
                    lat: this.props.poi.lat, 
                    long: this.props.poi.lng
                };
            }
    ...
    render() {
        ...
        const showRadius = results && this.props.searchType === "poi" && this.props.poi;
        ...
        return <GoogleMap
            ...
            >
            {
                results && 
                results.map(result=> {
                    return <PropertyMarker key={result._id} result={result} isSelected={selectedId==result._id} onSelect={this.handleSelect} limitValue={limitValue} minValue={minValue} maxValue={maxValue}/>
                })
       ...
       {/*           <DrawingManager
                    ...
                    onPolygonComplete={(value) => this.getPolygonPaths(value)}
                    onCircleComplete={(value) => this.handleCircle(value)}
                    /> */}
...
export const MapContainer = compose(
  ...
  export const MapContainerDashboard = compose(
    withProps({
      googleMapURL: `https://maps.googleapis.com/maps/api/js?key=${googleApiKey}&v=3.exp&libraries=geometry,drawing,places`,
      //googleMapURL: `https://maps.googleapis.com/maps/api/js?&v=3.exp&libraries=geometry,drawing,places`,
      loadingElement: <div style={{ height: `100%` }} />,
