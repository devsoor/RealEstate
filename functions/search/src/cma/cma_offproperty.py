...
class CompProperty(object):
    ...
    def __init__(self, listing, miles):
        ...
        self.year = int(listing.year_built)
...
class OffMarketSubjectProperty(object):
    resi_style_matches = [
        [0],		# Invalid style
    ...
    def __init__(self, listing, req_param_values): 
        self.listing = listing

        # Multi family
        if (listing["mp_style"] == 7):
            self.number_of_units = listing["units"]
            self.Units = []
            for unit_num in range(1, self.number_of_units+1):
                unit_current_rent = listing["rent"+str(unit_num)]
                unit_bed = float(listing["bed"+str(unit_num)])
                unit_bath = float(listing["bath"+str(unit_num)])
                unit_sqft = listing["sqft"+str(unit_num)]
            ...
            self.Current_Insurance = self.listing["Current_Insurance"]
            self.Current_ExpensesOther = self.listing["Current_ExpensesOther"]
            self.Current_PropertyTaxes = self.listing["Current_PropertyTaxes"]
            self.Current_WaterSewerGarbage = self.listing["Current_WaterSewerGarbage"]
            self.Current_TotalMonthlyIncome = self.listing["Current_TotalMonthlyIncome"]
            self.Current_GrossSchedIncome = self.listing["Current_GrossSchedIncome"]
            self.Current_EffectiveGrossIncome = self.listing["Current_EffectiveGrossIncome"]
            self.Current_GrossRentMultiplier = self.listing["Current_GrossRentMultiplier"]
            self.Current_NetOperatingIncome = self.listing["Current_NetOperatingIncome"]
            self.Current_PropertyMgmt = self.listing["Current_PropertyMgmt"]
            self.Current_HODAnnual = self.listing["Current_HODAnnual"]
            self.Current_VacancyRate = self.listing["Current_VacancyRate"]
        ...
        self.bath = float(listing["bathrooms"])
        self.zipcode = listing["zipcode"]
        self.sqft = int(listing["sqft"])
        self.purchase_price = float(listing["price"])
    ...
    def compute_cma_market_value(self, property_match_options, scoring_weights):        
        ...
        self.area_comps = OffMarketSubjectProperty.retrieve_area_comp_properties(self.style,
        ...
        if self.area_comps:
            ...
            area_comp_market = OffMarketSubjectProperty.perform_cma_analysis(self.style, self.year, self.sqft, self.bed, self.bath, self.area_comps, 
    ...
    def compute_cma_market_rent(self, style, mp_style, lat, lon, year, sqft, bed, bath, property_match_options, scoring_weights):
        ...
        self.area_rentals = OffMarketSubjectProperty.retrieve_area_rental_properties(style,
        ...
        if (self.area_rentals):
            ...
            area_rental_market = OffMarketSubjectProperty.perform_cma_analysis(style, year, sqft, bed, bath, self.area_rentals, property_match_options, scoring_weights)
    ...
    def retrieve_area_comp_properties(cls, style, mp_style, lat, lon, year, sqft, bath, bed, ptype, property_match_options):
        ...
        area_comps = OffMarketSubjectProperty.retrieve_area_properties(style, mp_style, lat, lon, year, sqft, bath, bed, ptype, status_list, property_match_options)
    ...
    def retrieve_area_rental_properties(cls, style, mp_style, lat, lon, year, sqft, bath, bed, property_match_options):
        ...
        area_rentals = OffMarketSubjectProperty.retrieve_area_properties(style, mp_style, lat, lon, year, sqft, bath, bed, 'rent', status_list, property_match_options)
    ...
    def retrieve_area_properties(cls, style, mp_style, lat, lon, year, sqft, bath, bed, ptype, status_list, property_match_options):
        ...
        if (style_match_type == 'Strict'):
            ...
            style_matches = OffMarketSubjectProperty.type_style_matches[ptype.lower()]
    ...
    def get_results(self, include_cma_props, include_area_props, pretty_print=False):
        ...
        if (self.mp_style == 7 and self.listing):
